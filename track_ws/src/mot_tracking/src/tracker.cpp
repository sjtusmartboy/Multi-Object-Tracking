                        ...
                        const Detection& _detections)
{
    ...
    for(const auto& detection : _detections){
        ...
        if(found){
            ...
            validationIdx++;//实际的关联矩阵的行数，也就是实际detection的数量
        }else{
                                           ...
                                           std::vector<track_ptr>& tracks_in)//TODO CHANGE THE FORM OF DETECTION
{
    ...
    // _association_matrices: [detection num, track num]
            ...
            if(mea_indicator == 1){
                ...
                for(uint notZero = 0; notZero < tracksize; ++notZero){
                    if(A_matrix(0, notZero) == 1){
                        ...
                        cv::Mat B;
                        cv::invert(S_cv, B);
//                        std::cout<<"ddd:  " << cv::format(B, cv::Formatter::FMT_DEFAULT) <<"::sss"<<std::endl;

                        const double& b = cv::Mahalanobis(z_cv, det_cv, B);
    ...
    for(uint i = 0; i < tracksize; ++i){
        for(uint j = 0; j < validationIdx; ++j){
            ...
            sumBeta(i) += beta(j, i); // sum over all measurement
        }
        sumBeta(i) = 1 - sumBeta(i); // track i is not associated with any measurement
    }
...
/**
 * @brief   manage_tracks
 * @details https://github.com/apennisi/jpdaf_trackingnot_associated_
 * @param   time
 */
void Tracker::manage_tracks(float& time){
    ...
    if(prevDetSize == 0){
        ...
        const uint& rows = assigmentsBin.rows; // 上一帧 detection 数量
        const uint& cols = assigmentsBin.cols; // 当前帧 detection 数量
        ...
        for(uint i = 0; i < dets.total(); ++i){
            ...
            prev_detections_.push_back(not_associated_.at(i));//把当前帧匹配的检测存起来
        }
    }
}
